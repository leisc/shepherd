...
|---|---|---|
| Python API reference (24 symbols) | **Real**: the `shepherd` integration facade (`shepherd/packages/meta/src/shepherd/__init__.py`), read statically by the generator; docstrings render from the actual runtime sources. | `generated`, internal build only. |
...
| Concepts (tasks, effects, runs, workspaces, providers) | Distilled from `docs/paradigm.md`, the spec, the curriculum, and the current facade pages. Public pages avoid planned-only permission, placement, workflow, and live-provider operations. | `shipped-source` / conceptual public pages |
